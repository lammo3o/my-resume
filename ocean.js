...
export function createOcean(){
 ...
 vec2 p=vec2(uv.x*aspect*2.8+time*.007,h*3.8);float n=fbm(p);float cloud=smoothstep(.49,.72,n)*smoothstep(.03,.22,h);col=mix(col,vec3(.99,.99,.94),cloud*.91);
 ...
 vec3 sea=mix(vec3(.19,.51,.65),vec3(.29,.75,.72),pow(depth,.6));sea+=(n-.5)*.15;
 ...
 sea=mix(sea,vec3(.98,.99,.93),clamp(foam,0.,.96));float shine=pow(max(0.,n),25.)*2.;sea+=shine;
