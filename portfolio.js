...
import {GLTFLoader} from './GLTFLoader.js?v=20260912show';
import {DRACOLoader} from './DRACOLoader.js?v=20260912show';
import {chooseQuality,TapGesture} from './compatibility.js?v=20260912show';
import {OrbitControls} from './OrbitControls.js?v=20260912show';
import {createOcean} from './ocean.js?v=20260912show';
import {lightPortrait} from './portrait-lighting.js?v=20260912show';
import {createPortraitLife} from './portrait-life.js?v=20260912show';
import {bindFace} from './avatar-surface.js?v=20260912show';
import {StickerLayer,stickerSVG,svgURL} from './stickers.js?v=20260912show';
import {DEFAULT_STICKERS,readPortfolio,isClick} from './portfolio-data.js?v=20260912show';
import {setupEditor} from './viewer.js?v=20260912show';
import {CameraStory} from './camera-story.js?v=20260912show';
...
const renderer=new THREE.WebGLRenderer({canvas,antialias:true});renderer.setPixelRatio(Math.min(devicePixelRatio,quality==='mobile'?1.5:2));renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.08;renderer.autoClear=false;
...
lightPortrait(renderer,scene,quality==='mobile');
const life=createPortraitLife();
let body,model,face,layer,stickers,restY=0,loaded=false,activeView='portrait',hoverId=null,placement=null,down=null,toastTimer,eyeFollow=true;
...
function frame(name){if(story.phase!=='idle')return;activeView=name;document.body.dataset.view=name;resize();const mobile=innerWidth<650;const views={portrait:[[0,mobile?.57:.77,mobile?2.65:2.62],[0,mobile?.52:.70,0]],half:[[0,0,mobile?5.5:4.7],[0,0,0]],back:[[0,.15,mobile?-5.5:-4.5],[0,.12,0]],face:[[0,.78,1.65],[0,.74,.04]]};const [p,t]=views[name];controls.enableDamping=false;controls.update();camera.position.fromArray(p);controls.target.fromArray(t);controls.update();controls.enableDamping=true;document.querySelectorAll('button[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===name));}
function resize(){renderer.setSize(innerWidth,innerHeight);camera.aspect=innerWidth/innerHeight;camera.clearViewOffset();if(innerWidth>1050&&activeView==='portrait')camera.setViewOffset(innerWidth,innerHeight,-innerWidth*.16,0,innerWidth,innerHeight);else if(innerWidth<650)camera.setViewOffset(innerWidth,innerHeight,0,-innerHeight*.055,innerWidth,innerHeight);camera.updateProjectionMatrix();ocean.uniforms.aspect.value=innerWidth/innerHeight;}
...
function waveButton(){$('motion').textContent=waves?'动画暂停':'动画播放';$('motion').setAttribute('aria-pressed',String(waves));}waveButton();$('motion').onclick=()=>{waves=!waves;waveButton();};
...
function quickNav(){const nav=$('quick-stickers');nav.replaceChildren();for(const s of stickers){const b=document.createElement('button'),img=document.createElement('img');img.src=svgURL(stickerSVG(s.kind));img.alt='';const label=document.createElement('span'),caption=document.createElement('span');label.textContent=s.label;caption.className='sticker-caption';caption.textContent='打开这段故事';b.append(img,label,caption);b.onclick=()=>openStory(s.id);nav.append(b);}}
...
new GLTFLoader().setDRACOLoader(draco).load(modelURL,async g=>{
 ...
 restY=model.position.y;face=bindFace(body,quality==='mobile',life.blink);layer=new StickerLayer(body);window.avatarBoot?.progress('正在贴上我的小故事…');
...
const clock=new THREE.Clock();renderer.setAnimationLoop(()=>{
 ...
 life.update(dt,waves&&story.phase==='idle');if(model){model.position.y=restY+life.breath;model.updateMatrixWorld(true);}
